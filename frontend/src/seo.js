...
export function seoHead(seo){
 ...
 meta('description',seo.description)+meta('robots',seo.robots)+meta('og:title',seo.title,true)+meta('og:description',seo.description,true)+meta('og:url',seo.url,true)+meta('og:type',seo.type,true)+meta('og:site_name',seo.name,true)+meta('og:image',seo.image,true)+meta('og:image:secure_url',seo.image?.startsWith('https:')?seo.image:undefined,true)+meta('og:image:alt',seo.image?seo.imageAlt:undefined,true)+meta('twitter:card',seo.image?'summary_large_image':'summary')+meta('twitter:title',seo.title)+meta('twitter:description',seo.description)+meta('twitter:image',seo.image)+meta('twitter:image:alt',seo.image?seo.imageAlt:undefined)+meta('article:published_time',seo.published,true)+meta('article:modified_time',seo.modified,true)+seo.schema.map(item=>`<script type="application/ld+json" data-site-seo>${serialize(item)}</script>`).join('')
}

...
4. Deploy the frontend as a Vercel project rooted at `frontend`. Its function needs the built `dist/index.html`, included by `frontend/vercel.json`. A static-only host needs an equivalent server renderer; `vite preview` alone does not exercise the production server function.
